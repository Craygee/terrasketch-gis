...
for (const [name, bounds] of studies) {
  for (const source of ['usgs-sites', 'twdb-wells', 'usgs-measurements', 'twdb-aquifers']) {
    ...
    assert.ok(['available', 'degraded', 'outside-coverage'].includes(r.status), `${name}/${source}: ${r.status}`);
    for (const record of r.records) { assert.ok(record.sourceUrl); assert.ok(record.retrievedAt); assert.ok(record.raw); }
    console.log(`${name}: ${source} ${r.status}, ${r.records.length} records, truncated=${r.truncated}`);
  }
}
