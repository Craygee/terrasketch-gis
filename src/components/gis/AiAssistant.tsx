...
function helpAnswer(prompt: string): string | null {
  ...
  if (/\b(add|create|make|load|import)\b.*\blayer\b/.test(prompt) && /\bedit/.test(prompt))
    return "Add a layer in any of three ways: open Public data and choose Add, drop a supported GIS file onto the Layers panel, or draw a point, line, or polygon and choose its destination. While drawing, use Add, Delete, Undo, Redo, Finish, or Cancel in the drawing bar. To reshape a saved feature, make its layer active, select it, choose Edit vertices, then use the Move, Add, Delete, Undo, Redo, Reset, Cancel, and Finish tools.";
  if (/\b(add|create|make|load)\b.*\blayer\b/.test(prompt))
    return "To add a layer, use Public data for an official dataset, drop GeoJSON/KML/KMZ/zipped Shapefile/GPX/CSV onto the Layers panel, or draw a feature. After drawing or selecting features, the destination dialog lets you create a new layer, choose its category and name, turn feature labels on and choose their attribute, or add the feature to an existing compatible layer.";
  if (/\b(edit|change|update)\b.*\b(feature|geometry|shape|attribute)\b/.test(prompt))
    return "Make the layer active and select the feature. Use Table to edit attribute values. Choose Edit vertices to open the shape-edit toolbar: Move drags vertices, Add inserts one on a highlighted edge, Delete removes a clicked vertex, Undo and Redo step through changes, Reset restores the starting shape, and Cancel exits without keeping the session's changes. Choose Finish when the shape is ready.";
  ...
  if (/\b(group|subgroup|folder|sublayer|feature list)\b/.test(prompt))
    return "Working layers, My sketches, Imported files, and Public data are layer groups. Use the pencil on a group header to rename any group, including the defaults. Check two or more layers and choose Group selected to place them together, or drag a layer or group by its handle to reorder it. Minimize all closes every open group, layer, sublayer repository, feature list, note, and editor in the sidebar. While dragging a layer, place it on a highlighted line between sibling layers to set its exact top-to-bottom draw order. Drop either a group or another layer on the middle of a layer row to place it inside that layer's Sublayers & groups section. Every nested item remains independently expandable, hideable and draggable, and a parent layer's eye controls its whole nested tree. Expand Feature sublayers separately to search, select, zoom, rename, hide, copy, move, or remove individual features.";
  if (/\b(vertex|vertices|reshape|edit geometry)\b/.test(prompt))
    return "Select one editable feature and choose Edit vertices. The compact toolbar lets you Move, Add, or Delete vertices, Undo or Redo changes, Reset to the starting geometry, Cancel the session, or Finish. Add works by clicking a highlighted edge; Delete works by clicking a white vertex. Lines retain at least 2 vertices and polygons at least 3.";
