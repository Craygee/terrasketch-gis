...
function helpAnswer(prompt: string): string | null {
  ...
  if (/\b(add|create|make|load|import)\b.*\blayer\b/.test(prompt) && /\bedit/.test(prompt))
    return "Add a layer in any of three ways: open Public data and choose Add, drop a supported GIS file onto the Layers panel, or draw a point, line, or polygon and choose its destination. While drawing, use Add, Delete, Undo, Finish, or Cancel in the drawing bar. To reshape a saved feature, make its layer active, select it, choose Edit vertices, then use the Move, Add, Delete, Undo, Reset, Cancel, and Finish tools.";
  ...
  if (/\b(edit|change|update)\b.*\b(feature|geometry|shape|attribute)\b/.test(prompt))
    return "Make the layer active and select the feature. Use Table to edit attribute values. Choose Edit vertices to open the shape-edit toolbar: Move drags vertices, Add inserts one on a highlighted edge, Delete removes a clicked vertex, Undo reverses the last change, Reset restores the starting shape, and Cancel exits without keeping the session's changes. Choose Finish when the shape is ready.";
